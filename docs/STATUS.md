...
- **The instance role cannot yet read `/commissionwatch/*`.** The role is `platform-aws-host`, shared with the other products on the box. An earlier version of this file said the grant "is not ours to make" — **that was wrong**; the account holder has `AdministratorAccess`, and an inline policy scoped to `/commissionwatch/*` touches no managed policy and no other tenant. See `deploy/README.md` §3. Until it lands, deploys run **DEGRADED** off the secret file already on the host and say so loudly on every run.
...
- **Nobody holds the plaintext of the env file.** It exists in the Gitea Actions secret, which is write-only, and in `/home/ec2-user/commissionwatch/.env` on the host. The host can seed Parameter Store from its own copy without anyone reading it — `deploy/README.md` §4.
