...
import { readQueueStats, readRunWork } from "../src/services/pressroom/queue-stats";
import {
  cleanupByPrefix,
  createJob,
  createRun,
...
describe("a sweep's own work versus somebody else's", () => {
  ...
  after(async () => {
    await cleanupByPrefix(OWN_PREFIX);
    // Closes the pool `../src/app` opens on import. One per file, in the last
    // suite — closing it in the first tore the pool down under this one.
    await db.destroy();
  });
